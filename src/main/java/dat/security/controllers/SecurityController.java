...
public class SecurityController implements ISecurityController {
    ...
    {
        this.securityDAO = new SecurityDAO(HibernateConfig.getEntityManagerFactory()); // Initialize ISecurityDAO
        this.dao = new SecurityDAO(HibernateConfig.getEntityManagerFactory()); // Initialize SecurityDAO
    }
    ...
    //Checks if the user is authenticated (Authorization Header Check and Token Verification)
    @Override
    ...
    {
        List<UserDTO> users = dao.getListOfUsers();
        // response
    ...
    public void getByUsername(Context ctx) {
        String username = ctx.pathParamAsClass("username", String.class).check(this::validatePrimaryKey, "Not a valid username").get();

        UserDTO userDTO = dao.getByUsername(username);
        // response
        ctx.res().setStatus(200);
    ...
    public void updateUser(Context ctx) {
        ...
        UserDTO userDTO = dao.updateUser(username, jsonRequest);
        // response
        ctx.res().setStatus(200);
    ...
    public boolean validatePrimaryKey(String username) {
        return dao.validatePrimaryKey(username);
    }



    //vi bruger ikke nedenstående, fungerer ikke helt som den skal. skaber problemer for updateUser()
    public UserDTO validateEntity(Context ctx) {
        return ctx.bodyValidator(UserDTO.class)
                .check( e -> e.getUsername() != null && !e.getUsername().isEmpty(), "Username must be set")
                .check( e -> e.getPassword() != null && !e.getPassword().isEmpty(), "Password must be set")
                .get();
    }
